...
#define DBG_MAX_VARS 32
#define DBG_MAX_STACK_DEPTH 32
...
typedef struct {
    ...
    // Previous registers (for change detection)
    struct {
        unsigned long rax, rbx, rcx, rdx;
        unsigned long rsi, rdi, rbp, rsp, rip;
    } prev_registers;
    ...
    // GDB MI process for variable tracking
    FILE *gdb_in;   // Write GDB commands here
    FILE *gdb_out;  // Read GDB output from here
    pid_t gdb_pid;

    // Variables tracking
    Variable variables[DBG_MAX_VARS];
    int variable_count;
...
void dbg_update_variables(Debugger *dbg);  // Update variable values
