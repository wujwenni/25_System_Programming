...
void ui_safe_print(WINDOW *win, int y, int x, const char *text) {
    ...
    // Ensure we don't print on borders
    ...
    // Print truncated text if needed
    char buffer[1024];
...
void ui_get_usable_area(WINDOW *win, int *start_y, int *start_x, int *height, int *width) {
    ...
    *start_y = 2;  // After border and title
    *start_x = 2;  // After border
    *height = max_y - 3;  // Exclude top border+title and bottom border
    *width = max_x - 3;   // Exclude left and right borders
